...
from models.instructions_processed_LP.ALFRED_task_helper import get_list_of_highlevel_actions, \
    determine_consecutive_interx, get_arguments, get_arguments_test, read_test_dict
...
class Sem_Exp_Env_Agent_Thor(ThorEnvCode):
    ...
    def load_initial_scene(self):
        ...
        try:
            ...
            self.picture_folder_name = "pictures/" + self.args.eval_split + "/" + self.args.dn + "/" + str(
                self.args.from_idx + self.scene_pointer * self.args.num_processes + self.rank) + "/"
            if self.args.save_pictures and not (
                    self.args.from_idx + self.scene_pointer * self.args.num_processes + self.rank in self.args.skip_indices):
            ...
            if self.args.test == False and not (self.args.valid_learned_lang):
                ...
                list_of_actions, categories_in_inst, second_object, caution_pointers = get_list_of_highlevel_actions(
                    traj_data, args_nonsliced=self.args.nonsliced)
            else:
                ...
                list_of_actions, categories_in_inst, second_object, caution_pointers = get_list_of_highlevel_actions(
                    traj_data, True, self.test_dict, self.args.nonsliced, self.args.appended)
            ...
            actions_dict = {'task_type': task_type, 'list_of_actions': list_of_actions, 'second_object': second_object,
                            'total_cat2idx': self.total_cat2idx, 'sliced': self.sliced, 'questions': questions}
    ...
    def move_until_visible(self):
        if not (self.rotate_aftersidestep is None) and abs(
                int(self.camera_horizon) - 0) < 5:  # rotated after sidestep but no longer visible
            obs, rew, done, info, success, _, target_instance, err, _ = self.set_back_to_angle(45)
        else:
    ...
    def plan_act_and_preprocess(self, planner_inputs, goal_spotted):
        ...
        if self.side_step_order in [1, 2]:
            ...
            if interaction is None:
                ...
                #if interaction == "PutObject":
                #    checker_verdict = self.task_checker(
                #        self.event.frame.copy(),
                #        planner_inputs["list_of_actions"][pointer],
                #        self.steps_taken,
                #        approximate_success=True,
            ...
            if self.prev_number_action == 0:  # stop outputted now or before
                if (
                    ...
                    if (
                        ...
                        if step_dir is None:
                            opp_step_dir = None
                            obs, rew, done, info, success, _, target_instance, err, _ = self.va_interact_new(
                                "LookUp_0")  # pass
                        else:
                            if step_dir == 'left':
                                opp_step_dir = 'right'
    ...
    def _plan(self, planner_inputs, step, goal_spotted, newly_goal_set):
        ...
        stg, stop, whether_real_goal = self._get_stg(map_pred, start, np.copy(goal),
                                                     planning_window, planner_inputs['found_goal'], xy.tolist(), step,
                                                     planner_inputs['exp_pred'], goal_spotted, newly_goal_set,
    ...
    def _get_stg(self, grid, start, goal, planning_window, found_goal, xy_forward, step, explored, goal_found,
                 newly_goal_set, pointer):
        def add_boundary(mat, value=1):
            h, w = mat.shape
            new_mat = np.zeros((h + 2, w + 2)) + value
            new_mat[1:h + 1, 1:w + 1] = mat
        ...
        planner_broken, where_connected = CH._planner_broken(planner.fmm_dist, goal, traversible, start,
        ...
        if planner_broken and self.steps > 1:
            ...
            if goal_found or self.args.use_sem_policy:
                if self.args.debug_local:
                    ...
                    self.print_log("Really broken?",
                                   CH._planner_broken(planner.fmm_dist, goal, traversible, start, self.steps_taken,
                                                      self.visited)[0])
                while CH._planner_broken(planner.fmm_dist, goal, traversible, start, self.steps_taken, self.visited)[
                    0] and (self.dilation_deg < d_threshold):
                    ...
                    if self.args.debug_local:
                        ...
                        self.print_log("Sanity check passed in loop is ",
                                       CH._planner_broken(planner.fmm_dist, goal, traversible, start, self.steps_taken,
                                                          self.visited)[0])
                ...
                if self.dilation_deg == d_threshold:
                    ...
                    np.random.seed(self.steps_taken)
                    random_goal_idx = np.random.choice(
                        len(where_connected[0]))  # choose a random goal among explored area
            ...
            if self.args.debug_local:
                self.print_log("Sanity check passed is ",
                               CH._planner_broken(planner.fmm_dist, goal, traversible, start, self.steps_taken,
                                                  self.visited)[0])

        if self.args.save_pictures:
            cv2.imwrite(self.picture_folder_name + "fmm_dist/" + "fmm_dist_" + str(self.steps_taken) + ".png",
                        planner.fmm_dist)
        ...
        stg_x, stg_y, _, stop = planner.get_short_term_goal(state, found_goal=found_goal,
    ...
    def depth_pred_later(self, sem_seg_pred):
        ...
        depth_img = pred_depth.get_trustworthy_depth(max_conf_int_width_prop=self.args.valts_trustworthy_prop,
                                                     include_mask=sem_seg_pred,
    ...
    def _visualize(self, inputs):
        ...
        if self.args.save_pictures:
            cv2.imwrite(self.picture_folder_name + "Sem_Map/" + "Sem_Map_" + str(self.steps_taken) + ".png",
                        self.semantic[:, :, [2, 1, 0, 3]])
